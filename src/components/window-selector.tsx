...
export default function WindowSelector() {
	...
	const createInitialWindows = useCallback(
		(deviceId: string | null): WindowContent[] => [
			{
				id: 1,
				title: "Dashboard",
				content: (
	...
	return (
		<div className="min-h-screen bg-gray-100 p-6">
			<div className="max-w-6xl mx-auto space-y-6">
				{/* Large Window */}
				<div
					className={`${activeWindow?.color} rounded-lg border-2 p-8 h-[500px] transition-all duration-300 ease-in-out`}
				>
				...
				<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
					{smallWindows.map((window) => (
						<div
							key={window.id}
							onKeyDown={() => handleWindowClick(window)}
				...
				<div className="flex">
					<button type="button" onKeyDown={handleRequestPermission}>
						Request Permission
					</button>
				</div>
			</div>
